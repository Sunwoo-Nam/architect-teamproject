...
def main():
    ...
    L = [f"# composite final 재집계 — {run_dir.name}", "",
         "판정: FC=달성률 평균 · RU=사용률 r (로그 사다리, 중앙+최악) · TB=ρ P95. "
    ...
    for dim, title, head in (("by_type", "유형별", "유형"),
        ...
        for k, per in bd[dim].items():
            body.append([k] + sum(([fmt(per[p], "fc_mean", "fc_stars"),
                                    ...
                                    fmt(per[p], "r_med", "ru_stars_med"),
                                    fmt(per[p], "r_max", "ru_stars_max")]
                                   for p in PLANS), []))
        L += md_table([head, "seq2 FC", "seq2 ρP95", "seq2 r중앙", "seq2 r최악",
                       "pool FC", "pool ρP95", "pool r중앙", "pool r최악"], body)
        L.append("")
