#!/usr/bin/env python3
"""QA 실측 기반 설명 그림 5종 — DP별 방안 비교 자료용 (2026-08-14 용도 변경).

61 브리핑의 QA 설명 그림은 **개념 도식(drawio — docs/changbae/drawio/61-*)으로 교체**됐다
(PL 지시: 실측 기반 그림은 DP별 방안 1·2 비교 때 사용). 본 스크립트는 그 비교 자료를
만들 때 재사용한다 — 산출물은 커밋되어 있지 않으며 필요 시 재생성한다.
