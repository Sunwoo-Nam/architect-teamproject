...
로더 의존 없음: `src/total/adapters/nparty/_vendor/benchmark.py`의 `validate_case`는
`benchmark-case.v1` 전용이고 스키마가 다르다. 또한 `issue-space-case.v1` 로더는 현재
별도 작업 중이므로, 본 생성기는 검증을 **스크립트 안에서 자체적으로** 수행한다.

사용 (poc/total 에서 실행):
    python scripts/generate_issue_space.py --track a      # 8단계 × 10건 = 80건 → cases/issue-space
    python scripts/generate_issue_space.py --track b      # 같은 규모 → cases/issue-space-b
    python scripts/generate_issue_space.py --per-config 2 # 축소 (단계당 2건)
    python scripts/generate_issue_space.py --steps 1 4 8  # 조합 64·1,728·62,208만
    python scripts/generate_issue_space.py --nested \\
        --participants 3 5 10                             # 참여자 수 비교용 (_build_all_nested 참고)

현재 datasets/nparty/cases/issue-space-n(480건)을 만든 명령. `--nested`는 트랙 A의 기본
출력 폴더(issue-space)를 쓰므로 `--out`을 반드시 지정해야 한다:
    python scripts/generate_issue_space.py --track a --nested \\
...
# [이식 시 수정] 출력 경로. 원본(poc/dp2-nparty/scripts)은 `parents[1] / "data" / "benchmark"
# / "cases"` 였다. 본 파일은 poc/total/scripts/ 에 있으므로 parents[1] 이 poc/total 이고,
# 데이터셋은 poc/total/datasets/nparty/cases 로 옮겨졌다. 이 상수 외의 생성 논리는
# 원본과 동일하게 두었다 — 같은 인자·같은 시드로 원본과 바이트 단위로 같은 결과가 나와야
# 이식이 성공했는지 대조할 수 있기 때문이다.
OUT_DIR = Path(__file__).resolve().parents[1] / "datasets" / "nparty" / "cases" / "issue-space"
...
# --- 의제 정의 -----------------------------------------------------------------
#
# 의제를 하나씩 붙여 조합 수를 8단계로 늘린다 (아래 ISSUE_LEVELS).
#
...
# 10.4초) 방안 2개만으로도 전체 실행이 3.5~7시간으로 추산됐다. 참여자를 10명으로 고정한다.
#
# 2026-08-12 사다리 개정: 위 축소로 남은 3단계(64 / 1,728 / 62,208)는 단계 사이가 27배·36배로
# 벌어져 "조합이 몇 개부터 방안이 갈리는가"를 볼 수 없었다. 의제를 **하나씩** 붙여 8단계로
# 촘촘하게 만든다 — 곱해지는 인수가 3,3,3,3,3,2,2라 조합 수가 고르게 올라가고, 기존 3단계가
# 그대로 남아 이전 측정과 비교가 유지된다.
#
# 실행 시간은 거의 전부 최상단에 몰린다 (실측: S=64 0.00초 · S=1,728 0.05초 · S=62,208 평균
# 9.0초 — 시간이 조합 수의 약 1.45제곱에 비례). 그래서 아래쪽 단계를 늘리는 비용은 작다.
#
# 한계: 이 사다리는 단계마다 의제 수(d)와 조합 수(S)가 함께 늘어난다. 따라서 "의제가 늘어난
# 효과"와 "조합이 늘어난 효과"를 분리하지 못한다 — 분리하려면 S를 고정하고 d만 바꾸는
# 교차 설계가 따로 필요하다 (미착수).
ISSUE_LEVELS = (
    (4, 4, 4),                              # d= 3 →      64
    (4, 4, 4, 3),                           # d= 4 →     192
    ...
    (4, 4, 4, 3, 3, 3, 3),                  # d= 7 →   5,184
    (4, 4, 4, 3, 3, 3, 3, 3),               # d= 8 →  15,552
    (4, 4, 4, 3, 3, 3, 3, 3, 2),            # d= 9 →  31,104
    (4, 4, 4, 3, 3, 3, 3, 3, 2, 2),         # d=10 →  62,208
)

PARTICIPANT_COUNTS = (10,)
PER_CONFIG = 10  # (의제 수 단계 × 참여자 수)당 케이스 수 → 8 × 1 × 10 = 80건/트랙
...
def main() -> None:
    ...
    print("  단계별 의제 구성 — 의제를 하나씩 붙여 조합 수를 2~3배씩 올린다")
