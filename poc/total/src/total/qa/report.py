...
def _section(qa: str, data: dict, plans: Sequence[str]) -> list[str]:
    ...
    keys: list[tuple[str, str]] = {
        ...
        "cf": [("exposure", "노출률 (판정)"), ("stars_exposure", "별점"),
               ("secret", "잔여 비밀률 (참고)"),
               ("m", "노출 배수 m (보조)"), ("stars_m", "m 별점")],
        "tb": [("p95_rho", "시간 비율 ρ P95 (판정)"), ("stars", "별점"),
               ("median_rho", "ρ 중앙값 (전형)"),
               ("max_rho", "최악 ρ"), ("defect_cases", "ρ>1 결함 케이스"),
               ("median_total_ms", "세션 시간 (ms)"), ("dominant", "지배 항"),
               ("median_phase_ms", "통신 항"), ("median_transfer_ms", "전송 항")],
        "ru": [("max_total_mb", "단말 총 점유 최대 (MB — 판정)"), ("stars_max", "별점"),
               ("p95_total_mb", "총 점유 P95 (MB)"), ("median_total_mb", "총 점유 중앙 (MB)"),
               ("median_peak_mb", "프로토콜 상태 (MB)"),
               ("median_materialized_mb", "실물화 후보 구조 (MB)"),
               ("median_base_mb", "공통 기저 (MB)"),
