...
def band_ru_usage(step: float = RU_STEP) -> Band:
    """사용률 r = 총점유 ÷ 한도의 별점 밴드 — 24 §2.8 (2026-08-14 반 데케이드 재개정).

    구 선형 등분(15%p)은 지수 성장 도메인에서 상·하 동시 포화했고(실측: 4~16축
    전부 ★5 → 20축 절벽), 1차 개정의 데케이드(×10) 사다리는 ★5 경계(0.01% =
    10,000배 여유)의 근거가 약했다 — "한도의 0.3%만 쓰는 설계가 ★3"이라는
    직관 불일치 (PL 지적 2026-08-14).

    **재개정: [1%, 100%]의 2데케이드를 로그 4등분** (반 데케이드 계단, PL 확정).
    참조점 둘 다 근거가 있다: 상한 = 한도(r=1, 결함 경계 — 불변), **★5 경계 =
    1% = 약 100배 여유** — 실측 축당 성장률 ×2.06-2.12(63 §B) 기준으로 기준
    시나리오(4축)가 실사용 최대(≈10축, SC-의제 사다리와 같은 앵커)까지 자라는
    성장 배수(2.1^6.2 ≈ 100)를 견디는 여유다. 즉 **★5 = "협상이 실사용 최대
    규모로 커져도 한도 안"**. 1칸 = √10 ≈ 3.2배 ≈ 의제 1.6개의 성장 여유.
    구 밴드는 `band_ru_usage_linear()`(선형) 병기용 보존.
    """
    return Band(
        name="RU 사용률 r (로그)",
        thresholds=[1e-2, 10 ** -1.5, 1e-1, 10 ** -0.5, 1.0],
        direction="at_most",
        note="24 §2.8 재개정 (2026-08-14) — r = 총점유 ÷ 한도, [1%,100%] 로그 4등분: "
             "≤1% ★5 · ≤3.2% ★4 · ≤10% ★3 · ≤32% ★2 · ≤100% ★1 · 초과 ★0(결함). "
             "★5 = 실사용 최대 규모(≈10축)까지의 성장(약 100배, 실측 ×2.1/축)을 "
             "견디는 여유. 1칸 = √10 ≈ 3.2배 ≈ 의제 1.6개",
    )
