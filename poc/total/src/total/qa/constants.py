...
#: 한도 대비 사용률의 등분 폭 — 20%p로 한도 전체를 5등분한다 (2026-08-14 PL 확정).
#: 구 15%p(초과분 >75% ★0에 여유를 둔 설계)는 이력 보존 (`band_ru_usage_linear(0.15)`).
RU_STEP = 0.20


def band_ru_usage(step: float = RU_STEP) -> Band:
    """사용률 r = 총점유 ÷ 한도의 별점 밴드 — 24 §2.8 (2026-08-14 20%p 등분 확정).

    **확정: [0,100%]의 20%p 등분** (PL 확정 2026-08-14) — ≤20% ★5 · ≤40% ★4 ·
    ≤60% ★3 · ≤80% ★2 · ≤100% ★1 · 초과 ★0(결함). TB(ρ 0.2 등분)·CF(노출률
    20%p 등분)와 **눈금이 통일**되어 세 QA의 사다리를 같은 자로 읽는다.

    인지된 트레이드오프 (개정 기록 — 24 §2.8): r는 자릿수 단위로 퍼지는 지표라
    선형 등분은 저사용 구간을 ★5 하나로 묶는다 (실측: seq2 0.96%와 P6 4.0%가
    동급 ★5 — 여유 4배 차이는 원값 병기로 읽는다). 이 트레이드오프는 눈금 통일을
    우선한 PL 결정으로 수용됐다. 직전 로그 사다리([1%,100%] 4등분)는
    `band_ru_usage_log()`로 이력 보존 — ★5 = 1%의 "실사용 최대까지 성장 100배
    여유" 앵커 해석은 그쪽 기록 참조.
    """
    return Band(
        name="RU 사용률 r (20%p)",
        ...
        direction="at_most",
        note="24 §2.8 확정 (2026-08-14, PL) — r = 총점유 ÷ 한도, 20%p 등분: "
             "≤20% ★5 · ≤40% ★4 · ≤60% ★3 · ≤80% ★2 · ≤100% ★1 · 초과 ★0(결함). "
             "TB·CF와 동일 눈금. 저사용 구간의 변별은 원값(r %) 병기로 읽는다",
    )


def band_ru_usage_log() -> Band:
    """구 로그([1%,100%] 반 데케이드) 밴드 — 이력 대조용 (2026-08-14 개정 2차분).

    ★5 경계 1% = 실측 축당 성장률 ×2.06-2.12 기준 실사용 최대(≈10축)까지의 성장
    배수(약 100배)를 견디는 여유 — 저사용 구간 변별이 필요할 때 참조한다.
    """
    return Band(
        name="RU 사용률 r (로그·구)",
        thresholds=[1e-2, 10 ** -1.5, 1e-1, 10 ** -0.5, 1.0],
        direction="at_most",
        note="구 24 §2.8 (2026-08-14 반 데케이드) — [1%,100%] 로그 4등분. "
             "20%p 등분 확정으로 판정에서 내림 (눈금 통일 우선)",
    )
