...
| **RS** (n=12-20) | 2.5×10^6 - 6.1×10^11 | RU + TB만 — FC 필드는 `null` 명시 | 전수 열거 불가 — "안 쟀다"를 null로 구분 (0·생략과 다름) |
...
- **P95 규약** = 정렬 후 0-기반 색인 int(0.95·n) — nearest-rank보다 1랭크 위를 취하는 상향 보수. 측정 파이프라인(qa/tb.py·qa/ru.py)과 재집계기가 동일 규약을 쓴다 (2026-08-14 통일 — 제3자 리뷰에서 이원화 발견). **n ≤ 20 부분표본에서는 최댓값과 일치**하므로 소표본 분해표는 "최악"으로 표기한다.
