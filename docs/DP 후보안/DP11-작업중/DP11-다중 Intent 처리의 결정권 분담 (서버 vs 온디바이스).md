...
- **Silent failure 위험 — 폴백으로 해결되지 않는 구조적 약점**. Policy Table lookup이 *hit*하지만 *틀린 룰*을 반환하면 폴백이 트리거되지 않아(폴백은 *miss*만 잡음) 잘못된 액션이 dispatch됨.
- 정책표에 *없는* 룰(미상의 라벨 조합·실패 양상)이 등장하면 폴백 호출이 필요. 초기 Skeleton 품질이 낮거나 운영 중 폴백 빈도가 예상보다 높으면 사용자 체감 응답성·배터리 강점이 약화되고, 결국 Sol.1과 비슷한 비용으로 수렴할 수 있음.
