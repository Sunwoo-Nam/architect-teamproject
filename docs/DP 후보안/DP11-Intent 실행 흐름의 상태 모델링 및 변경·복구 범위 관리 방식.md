...
flowchart TB
    ...
    O --> ST["Current State Table<br/>Intent별 현재 상태"]
    O --> RM["Simple Relation Metadata<br/>priority / supersedes / blocked-by"]

    ST --> D["실행 판단<br/>queue / running / completed / failed / canceled"]
    RM -. "관계 표현은 제한적" .-> D
...
이 흐름에서는 새로운 Intent가 들어오면 Orchestrator가 해당 Intent의 현재 상태를 `Current State Table`에 기록합니다.
Intent 간 관계는 별도의 graph가 아니라 `priority`, `supersedes`, `blocked-by` 같은 단순 metadata로만 보조 표현합니다.
이후 Orchestrator는 각 Intent의 현재 상태를 기준으로 실행할지, 대기시킬지, 완료/실패/취소 처리할지를 결정합니다.
...
flowchart TB
    ...
    G --> R1["Intent 관계 판단<br/>merge / supersedes / dependency / conflict"]
    G --> R2["외부 상태 반영 범위<br/>Device / Calendar / External API"]
    G --> R3["실행 이력<br/>누가 / 언제 / 무엇을 수행했는지"]

    R1 --> D["실행 계획 및 복구 범위 판단"]
    R2 --> D
    R3 --> D
...
시스템은 event log를 그대로 현재 상태로 보지 않고, 이를 `Projection` 과정을 통해 `Task Graph`로 재구성합니다.
...
실패, 취소, 변경이 발생하면 그 사실 역시 event로 기록되고, 시스템은 graph와 실행 이력을 바탕으로 실행 계획과 복구 범위를 다시 계산합니다.
