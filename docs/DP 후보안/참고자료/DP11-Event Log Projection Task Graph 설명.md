...
Projection은 LLM이 수행하는 의미 해석 과정이라기보다, 정해진 event schema와 규칙에 따라 현재 graph를 재구성하는 deterministic 처리로 두는 것이 적합합니다.
복구 가능성과 재현성을 보장하려면 같은 Event Log를 입력했을 때 항상 같은 Projection 결과가 나와야 하기 때문입니다.
...
구현 관점에서는 event replay와 view materialization에 가깝습니다.

### Projection은 코드로 가능한가?

Projection은 코드로 구현할 수 있으며, 오히려 코드로 구현하는 것이 적합합니다.
Projection은 LLM이 의미를 새로 판단하는 과정이 아니라, 이미 구조화되어 저장된 Event Log를 정해진 규칙대로 읽어서 현재 상태 view를 만드는 과정이기 때문입니다.

예를 들어 Event Log가 다음과 같이 구조화되어 있다고 가정할 수 있습니다.

```text
1. IntentReceived(intentId=A, goal="토요일 저녁 식사 예약")
2. IntentUpdated(intentId=A, condition="아이 동반 가능")
3. IntentBlocked(intentId=A, reason="토요일 저녁 일정 충돌")
4. IntentSuperseded(oldIntent=A, newIntent=B)
5. ExternalStateChanged(taskId=T1, resource="Calendar", action="temporary_block")
```

Projection 코드는 이 event들을 순서대로 읽으면서 현재 view를 만듭니다.

```text
Intent A:
  status = superseded
  conditions = ["아이 동반 가능"]
  blockedBy = "토요일 저녁 일정 충돌"
  externalEffects = ["Calendar temporary_block"]
...
function project(events) {
  ...
  for (const event of events) {
    switch (event.type) {
      ...
      case "IntentUpdated":
        view.intents[event.intentId].conditions.push(event.condition);
        break;

      case "IntentBlocked":
        view.intents[event.intentId].status = "blocked";
        view.intents[event.intentId].blockedBy = event.reason;
        break;

      case "IntentSuperseded":
        view.intents[event.oldIntent].status = "superseded";
        view.intents[event.newIntent] = {
          status: "active",
          supersedes: event.oldIntent,
          conditions: [...view.intents[event.oldIntent].conditions]
        };
        break;

      case "ExternalStateChanged":
        view.resources[event.resource] = {
