...
- **구조**: 메인 프로세스는 B(감독자 트리)로 운영하고, **분리 자격(아래 결정 트리)이 있는 컴포넌트만 별도 OS 프로세스**로 분리. 본 프로젝트의 분리 후보는 **(a) LLM 추론(Model-Server 프로세스)**, **(b) Accessibility Service**(Android 정책상 별도 프로세스 가능 여부는 5절 미결 변수), **(c) MAF**(Meta·Sub·Orchestrator — IDS와의 생애주기 비대칭으로 분리). 분리 경계는 *3개 내외*로 한정.
...
- **분리 자격 결정 트리** (모듈을 분리할지 판단할 때 적용):
  ...
  회수/메모리/격리 이득이 큰가?
    ...
    → 아니오 → 같은 프로세스 유지
  ```
  → 본 프로젝트에서 분리 결정된 모듈: **LLM 추론**(자격 A, 거의 확정), **Accessibility Service**(자격 A, 분리 가능성 검증 후 결정 — 5절), **MAF**(자격 B, idle 메모리·수명 관리 정책 측정 후 확정 — 5절).

- **장점**
  - **취소 불가 네이티브 멈춤 회수 가능** → **E3 ★★★** (B의 ★★ 한계 돌파, 자격 A 분리분 효과).
  - **MAF idle 시 메모리 결정론적 회수** → **E1 부분 개선** (자격 B 분리분 효과 — 같은 프로세스 lazy load는 GC가 확률적인데 별도 프로세스는 OS가 즉시 회수).
  - **IDS의 누적 perception 상태 보존** → MAF가 죽어도 IDS는 계속 동작, 누적 정보 손실 없음 (자격 B 분리분 효과).
  - 분리된 컴포넌트의 멈춤이 다른 프로세스를 끌고 가지 않음 → **E4 ★★** (분리 영역에 한해 격리).
  - 일반 모듈은 여전히 단일 프로세스 안의 감독자 트리에 머묾, 자원·일관성 부담은 *분리 경계 K개*에만 비례 (전면 분리 대비 압도적으로 작음).
  - 산업 표준 패턴(Chrome·Android 시스템 서비스·Erlang)으로 검증됨.
- **단점**
  - 분리 경계 K개당 비용: AIDL/Binder 인터페이스, 감시자 응답 확인(ping), 재시작 폭주 방지, Model-Server 프로세스의 모델 로딩 수명 관리, **MAF 수명 관리 정책(언제 깨우고 죽일지)** → **E1·E2 ★★** (B보다 부담↑).
  - 분리 경계에서의 시점 동기화 비용은 *경계 수에 비례* — K=3이라 누적 부담은 있지만 전면 분리보다 훨씬 작음 → **E6 ★★**.
  - **IDS↔MAF 잦은 호출 시 IPC 지연 누적** — QAS-002(≤180s) 영향 가능 → 5절 미결 변수에서 측정 필요.
  - **감시자 구현 부담(중)**: B의 외부 감시자 + 같은 프로세스 안 감독자 + 분리 컴포넌트별 AIDL 응답 확인 감시자 (분리 경계 K개).
...
- **IDS는 메인 프로세스에 상주** (perception data 누적, 사용자 요청 대기 — 데몬형)
- **MAF(Meta·Sub·Orchestrator)는 별도 프로세스로 분리** — 협상 같은 태스크 시작 시 깨우고 끝나면 정리. IDS의 상시성과 MAF의 일과성을 같은 컨테이너에 묶지 않음 (자격 B)
- **LLM 추론·Accessibility는 별도 OS 프로세스로 분리** — 취소 불가 네이티브 멈춤을 강제 종료로 회수 가능하게 함 (자격 A)
- 총 분리 경계 K=3 내외 (LLM, Accessibility, MAF)
- 분리 경계 비용(E1·E2·E6 각 ★★)이 누적되지만, K=3라 한정적
- 전면 분리(이전 검토 안)의 자기모순(Android 메모리 부족 시 자동 종료(LMK) 충돌, 자원 폭증, 일관성 붕괴)은 발생하지 않음

### 4.3 산업 표준과의 일관성

C가 채택하는 *선택적 분리(격벽 패턴, bulkhead pattern)* 는 다음과 동일하게 쓰이는 패턴이다:

| 시스템 | 같은 사고 |
|---|---|
| **Chrome** | 사이트별 격리(자격 A) + network process(상시) vs renderer(태스크형, 자격 B) |
| **Android** | 시스템 서비스(mediaserver·audioserver·surfaceflinger) 별도 proc + system_server(상시) vs 앱 proc(요청형) |
| **Erlang OTP** | 네이티브 함수(NIF) 분리(자격 A) + application supervisor(상시) vs job workers(태스크형) |
| **마이크로서비스** | background workers(상시) vs request handlers(요청형) |

자격 A(취소 불가 네이티브 멈춤만 분리)는 전제 4(*"OS 프로세스 분리가 유일한 회수 수단인 경우는 취소 불가능한 네이티브 멈춤뿐"*)에 직접 근거한다. 자격 B(생애주기 비대칭으로 분리)는 *자원 격리*(메모리 회수 보장)와 *운영 격리*(태스크형 모듈이 상시형 모듈의 안정성을 해치지 않게)에 근거한다.
