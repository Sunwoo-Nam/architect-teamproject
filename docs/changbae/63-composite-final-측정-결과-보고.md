...
> **갱신 2차 (2026-08-14)**: RU **판정 통계를 P95에서 최대값으로 교체** (24 §2.8 2차 재개정, PL 확정 — OOM은 최악 1건이 크리티컬; TB는 체감 품질이라 P95 유지). 측정값 불변, 판정 표기만 최대값 기준: seq2 **max 0.96% ★5**(불변) · pool **max 3,692% ★0**(P95 48.2% ★1 → 최악 기준 — 초과 11건과 정합). P95·중앙값은 병기로 강등.
...
| **2. 자원(메모리)** — 단말 총 점유÷한도 r의 최대값 | **0.96% ★5** | 3,692% ★0 | **방안 1** (한도 초과 0 vs 11건) |
...
2. **메모리는 방안 1의 구조적 우위 — 규모가 커질수록 절대적.** 방안 1의 단말 총 점유는 최악 1.2MB(r 최악 0.96% — 판정값)·P95 0.39MB — 전 구간 한도의 1% 미만이다. 방안 2는 압축 풀의 지수 성장으로 12축 0.5MB → 18축 42MB → 20축 110MB(중앙)이고, 18-20축에서 **한도(128MB) 초과 11건**(최악 4.7GB — k 심화 폭주 꼬리)이 나왔다. 소규모(CR)에서는 오히려 방안 2가 더 가볍다(쌍대 중앙 0.55배) — 중앙값 기준 역전은 11축부터이고, 꼬리(P95) 기준으로는 8축부터 방안 2가 무거워진다(E표).
...
| RU | **사용률 r** = 단말 총 점유 ÷ 협상 몫 한도(128MB) — 케이스 **최대값** (OOM은 최악 1건이 크리티컬 — P95·중앙 병기) | ≤1% ★5 / ≤3.2% ★4 / ≤10% ★3 / ≤32% ★2 / ≤100% ★1 / 초과 결함 | [1%,100%] 로그 4등분 — ★5 = 실사용 최대까지 성장(약 100배) 여유, 1칸 = √10 ≈ 3.2배 | 2026-08-14 |
...
| FR 위반 케이스 | 0 | 0 |
| **RU r 최대 (판정)** | 0.96% **★5** | 3,692% **★0** |
| RU r P95 / 중앙 | 0.30% ★5 / 0.15% ★5 | 48.2% ★1 / 0.12% ★5 |
...
- RU에서 두 방안의 **중앙값은 동급 ★5**다 — 격차는 전부 규모 꼬리(P95·최악)에 있고, 그것이 곧 "한 순간의 최악으로 죽는" OS 관점의 리스크다 (24 §2.6·§2.8 — 판정을 최대값으로 두는 이유가 정확히 이것이다).
...
RS 트랙 (경량 — RU·TB, 수준당 20건 — 최악값 표기. RU는 판정 통계 자체가 최대값(24 §2.8 2차)이고, TB는 수준당 표본 20건이라 P95 규약(정렬 색인 int(0.95n))상 최악과 일치한다. 전체 표본 판정은 D1이 정본):
...
- **Q. RU 판정을 최대값·로그 사다리로 바꾼 것이 방안 2에 불리한 조작 아닌가?** — 사다리 개정은 측정 전에 24 §2.8에 문서화됐고(근거: 선형 등분의 상·하 포화 실측), 판정 통계의 최대값 교체(2026-08-14 2차)는 QA의 성격(OOM = 크리티컬 사건은 최악 기준, TB = 체감 품질은 P95)에서 나온 규칙이지 방안을 겨냥한 것이 아니다 — 중앙값 판정이면 두 방안이 ★5 동급으로 뭉개져 방안 2의 실측 한도 초과 11건이 별점에 안 보이게 된다. 그쪽이 조작이다.
...
- **Q. 핸드북의 목표치(★5 기준) 대비로는 두 방안 다 미달 아닌가?** — 맞다, 그리고 감추지 않는다: 승자인 방안 2도 FC 0.941(★5 기준 0.95), r 최대 3,692%(★5 기준 1% — 한도 초과 결함), ρ P95 0.633(기준 0.2)로 전부 ★5 미달이다. 24 §0의 ★5 기준은 합격선이 아니라 사다리의 최상 등급이고, 본 측정의 목적은 절대 합격 판정이 아니라 **DP 대안 비교**다 — 별점이 목표까지의 거리를 표현한다. 구간별로 보면: TB ★5는 조합이 작은 판에서 naive 자체가 빠른 탓에 구조적으로 어렵고 규모가 커지면 두 방안 모두 도달한다(RS 18축부터 최악 ρ ≤ 0.003). FC·RU의 잔여 갭은 보완 택틱([`62`](62-방안2-보완-택틱-설계-측정.md))과 k 상한·의제 분할(K2)의 과제로 남긴다.
