...
| **2. 자원(메모리)** — 단말 총 점유÷한도 r의 P95 | **0.30% ★5** | 48.2% ★1 | **방안 1** (한도 초과 0 vs 11건) |
...
| RU | **사용률 r** = 단말 총 점유 ÷ 협상 몫 한도(128MB) — 케이스 **P95** | ≤1% ★5 / ≤3.2% ★4 / ≤10% ★3 / ≤32% ★2 / ≤100% ★1 / 초과 결함 | [1%,100%] 로그 4등분 — ★5 = 실사용 최대까지 성장(약 100배) 여유, 1칸 = √10 ≈ 3.2배 | 2026-08-14 |
...
| FR 위반 케이스 | 0 | 0 |
| **RU r P95 (판정)** | 0.30% **★5** | 48.2% **★1** |
| RU r 중앙 / 최악 | 0.15% ★5 / 0.96% ★5 | 0.12% ★5 / 3,692% ★0 |
...
- RU에서 두 방안의 **중앙값은 동급 ★5**다 — 격차는 전부 규모 꼬리(P95·최악)에 있고, 그것이 곧 "한 순간의 최악으로 죽는" OS 관점의 리스크다 (24 §2.6·§2.8).
...
- **Q. 핸드북의 목표치(★5 기준) 대비로는 두 방안 다 미달 아닌가?** — 맞다, 그리고 감추지 않는다: 승자인 방안 2도 FC 0.941(★5 기준 0.95), r P95 48.2%(★5 기준 1%), ρ P95 0.633(기준 0.2)로 전부 ★5 미달이다. 24 §0의 ★5 기준은 합격선이 아니라 사다리의 최상 등급이고, 본 측정의 목적은 절대 합격 판정이 아니라 **DP 대안 비교**다 — 별점이 목표까지의 거리를 표현한다. 구간별로 보면: TB ★5는 조합이 작은 판에서 naive 자체가 빠른 탓에 구조적으로 어렵고 규모가 커지면 두 방안 모두 도달한다(RS 18축부터 최악 ρ ≤ 0.003). FC·RU의 잔여 갭은 보완 택틱([`62`](62-방안2-보완-택틱-설계-측정.md))과 k 상한·의제 분할(K2)의 과제로 남긴다.
