...
> **판정 체계**: FC = 달성률 · RU = 사용률 r **최대값**(로그 사다리 — 2026-08-14 2차 재개정) · TB = ρ P95. 본 실행이 이 체계의 최종 정본 측정이다 (측정 자체는 2026-08-13 P95 체계로 수행 — 아래 갱신 2차 참조).
> **갱신 (2026-08-14)**: RU 사다리 재개정(24 §2.8 — [1%,100%] 로그 4등분, ★5 = 1%) 반영 — RU 별점만 재환산(측정값 불변): seq2 P95 0.30% ★3→**★5**, 중앙/최악 별점도 갱신.
> **갱신 2차 (2026-08-14)**: RU **판정 통계를 P95에서 최대값으로 교체** (24 §2.8 2차 재개정, PL 확정 — OOM은 최악 1건이 크리티컬; TB는 체감 품질이라 P95 유지).
> **갱신 3차 (2026-08-14, PL 지시)**: RU 별점을 **P95·최대값 2케이스 병행 산출·보고**로 확장 (24 §2.8 3차). 단일 인용 시 대표는 최대값(보수 기준). 측정값은 전 기간 불변.
> **갱신 4차 (2026-08-14, PL 확정)**: RU 별점 사다리를 **[0,100%] 20%p 등분**으로 확정 (24 §2.8 — TB·CF와 눈금 통일, 저사용 변별은 원값 병기). 재환산: 케이스 1 P95 = seq2 0.30% ★5 / pool 48.2% **★3**(구 로그 ★1), 케이스 2 최대 = seq2 0.96% ★5 / pool 3,692% ★0(불변 — 한도 초과).
...
| **2. 자원(메모리)** — r 케이스 1: **P95** | **0.30% ★5** | 48.2% ★3 | **방안 1** |
| **2. 자원(메모리)** — r 케이스 2: **최대값** (단일 인용 대표) | **0.96% ★5** | 3,692% ★0 | **방안 1** (한도 초과 0 vs 11건) |
...
2. **메모리는 방안 1의 구조적 우위 — 규모가 커질수록 절대적.** 방안 1의 단말 총 점유는 최악 1.2MB(r 0.96%)·P95 0.39MB(r 0.30%) — 두 케이스 모두 한도의 1% 미만(★5)이다. 방안 2는 압축 풀의 지수 성장으로 12축 0.5MB → 18축 42MB → 20축 110MB(중앙)이고, 18-20축에서 **한도(128MB) 초과 11건**(최악 4.7GB — k 심화 폭주 꼬리)이 나왔다. 소규모(CR)에서는 오히려 방안 2가 더 가볍다(쌍대 중앙 0.55배) — 중앙값 기준 역전은 11축부터이고, 꼬리(P95) 기준으로는 8축부터 방안 2가 무거워진다(E표).
...
| RU | **사용률 r** = 단말 총 점유 ÷ 협상 몫 한도(128MB) — 별점은 **P95·최대값 2케이스 병행** (케이스 1 P95 = 꼬리 여유 · 케이스 2 최대 = 최악 안전, OOM 크리티컬. 단일 인용 대표 = 최대값) | ≤20% ★5 / ≤40% ★4 / ≤60% ★3 / ≤80% ★2 / ≤100% ★1 / 초과 결함 | **20%p 등분** — TB·CF와 눈금 통일 (저사용 구간 변별은 원값 병기 — 24 §2.8 트레이드오프 기록) | 2026-08-14 |
...
- **로그 사다리**: 조합 공간·실물화 구조는 의제 수에 지수 성장(축수 스윕 실측 축당 ×2.06-2.12 — [`01-축수-스윕-RU-분석.md`](../../poc/total/results/composite-per-case/01-축수-스윕-RU-분석.md))하므로 선형 15%p 등분은 상·하 포화한다 (실측: 4-16축 전부 ★5 → 20축 ★0 절벽). 재개정(2026-08-14, 24 §2.8): [1%,100%] 로그 4등분 — ★5(≤1%) = 실사용 최대(≈10축)까지의 성장(약 100배, 실측 ×2.1/축)을 견디는 여유, 1칸 = √10 ≈ 3.2배 ≈ 의제 1.6개. 참조점(한도 r=1)은 불변.
...
| FR 위반 케이스 | 0 | 0 |
| **RU r — 케이스 1: P95** | 0.30% **★5** | 48.2% **★3** |
| **RU r — 케이스 2: 최대값** (단일 인용 대표) | 0.96% **★5** | 3,692% **★0** |
| RU r 중앙 (참고) | 0.15% ★5 | 0.12% ★5 |
...
- RU에서 두 방안의 **중앙값은 동급 ★5**다 — 격차는 전부 규모 꼬리(P95·최악)에 있다. 두 케이스가 같은 방향을 가리키되(방안 1 우세), pool은 케이스 1(★3)과 케이스 2(★0 — "한 순간의 최악으로 죽는" OS 관점)의 간극으로 리스크의 성격까지 드러난다 (24 §2.6·§2.8).
...
CR 트랙 (오라클 판정 — FC·ρP95·rP95. r의 표본 판정은 최대값이나, 축 수준별 분해는 꼬리 추세 관찰용으로 P95를 유지 — 수준당 44-45건):
...
RS 트랙 (경량 — RU·TB, 수준당 20건 — 최악값 표기. RU는 케이스 2(최대) 통계와 일치하고, TB는 수준당 표본 20건이라 P95 규약(정렬 색인 int(0.95n))상 최악과 일치한다. 전체 표본 판정은 D1이 정본):
...
**그림 H-2a — RU: 사용률 r의 누적분포** (500건/방안 — 두 케이스의 판정점을 함께 표시: P95 = 점선 높이 0.95의 교점, 최대 = 곡선 끝점). 방안 1(파랑) 곡선이 10^-2 부근에서 끝나는 것은 데이터 절단이 아니라 **최악값 자체가 0.96%** 이기 때문이다 — ECDF는 표본 최댓값에서 1.0에 도달하며 끝난다:
...
| 방안 | p50 | p90 | **p95 (케이스 1)** | **최악 (케이스 2)** |
|---|---|---|---|---|
| seq2 | 0.15% | 0.26% | **0.30% ★5** | **0.96% ★5** |
| pool | 0.12% | 6.6% | **48.2% ★3** | **3,692% ★0** |

- **seq2의 ρ는 p75에서 이미 1을 넘는다** — "네 판에 한 판은 naive보다 느리다"가 중앙값(0.43)만 보면 안 보인다. 판정을 P95로 두는 이유.
- **pool의 r는 p50(0.12%)과 최악(3,692%)이 5개 자릿수 차이** — 중앙값 판정이면 두 방안이 동급 ★5로 뭉개진다. 최악 3,692%(≈4.7GB)는 18축 케이스에서 k 심화가 폭주한 꼬리로, "평소엔 가볍고 어느 순간 죽는다"는 압축 풀의 리스크 프로파일 그 자체다 — **판정을 최대값으로 두는 이유** (24 §2.8 2차).
- 현행 20%p 등분 사다리(2026-08-14 확정)에서는 저사용 구간이 ★5 한 칸에 모인다 — 자릿수 단위의 저사용 변별(예: seq2 0.96% vs P6 4.0%)은 별점이 아니라 위 분위표의 원값으로 읽는다 (사다리 이력·트레이드오프 기록: 24 §2.8).
...
- **Q. 세션 상한 60스텝은 자의적이지 않나? 상한 때문에 죽는 합의는 얼마나 되나?** — 하네스 기본 200이야말로 근거 없는 임의값이었다. 75ms 페이즈 기준 200스텝은 순수 프로토콜만 30초로 모바일 세션에서 비현실적이다. 60스텝(≈9초)을 두 방안에 같은 단위(협상 세션당)로 적용했고, boulware 양보가 상한에 비례하므로 이는 "데드라인 안에서 얼마나 잘 협상하는가"라는 현실 질문이다. naive 기준선은 비용의 정의이므로 자르지 않는다 (A4). **상한의 실제 비용은 실측했다** (PL 질의 2026-08-14 — 정본의 결렬 전 케이스를 상한 120·240으로 재실행): 상한 완화로 합의로 전환되는 케이스는 **방안 2가 6건/500(1.2% — plain 1·stress 5), 방안 1은 0건**이다. 방안 1의 결렬 55건(함정 43 포함)은 상한을 4배로 늘려도 전부 결렬 — 원인이 라운드 부족이 아니라 **백트랙 메커니즘 자체의 회복 실패**임을 확인한다(경로 의존이 진짜 원인이라는 F절 해석의 독립 검증). 전환 6건 중 CR은 1건뿐이고 그 합의 품질도 달성률 0.617로 낮다("간신히 되는 나쁜 합의"). 결렬 정답 45건×2방안은 상한 240에서도 전부 정확히 결렬(억지 합의 0 — 새니티 통과).
- **Q. RU 판정 통계·사다리를 여러 번 바꾼 것이 방안 2에 불리한 조작 아닌가?** — 개정은 전부 24 §2.8에 근거와 함께 기록됐고, 방향도 일관되게 방안을 겨냥하지 않았다: 판정 통계(P95·최대 2케이스 병행)는 QA의 성격(OOM = 크리티컬은 최악 기준, TB = 체감은 P95)에서 나온 규칙이고, 최종 사다리(20%p 등분, 2026-08-14)는 오히려 방안 2의 P95 등급을 로그 사다리 대비 ★1→★3으로 **올리는** 방향이었다. 두 케이스를 나란히 공개하므로 어느 한 통계의 선택이 결과를 왜곡할 여지도 없다 — 중앙값 판정이면 두 방안이 ★5 동급으로 뭉개져 방안 2의 실측 한도 초과 11건이 별점에 안 보이게 된다. 그쪽이 조작이다.
...
- **Q. 핸드북의 목표치(★5 기준) 대비로는 두 방안 다 미달 아닌가?** — 맞다, 그리고 감추지 않는다: 승자인 방안 2도 FC 0.941(★5 기준 0.95), r P95 48.2%·최대 3,692%(★5 기준 1% — 한도 초과 결함), ρ P95 0.633(기준 0.2)로 전부 ★5 미달이다. 24 §0의 ★5 기준은 합격선이 아니라 사다리의 최상 등급이고, 본 측정의 목적은 절대 합격 판정이 아니라 **DP 대안 비교**다 — 별점이 목표까지의 거리를 표현한다. 구간별로 보면: TB ★5는 조합이 작은 판에서 naive 자체가 빠른 탓에 구조적으로 어렵고 규모가 커지면 두 방안 모두 도달한다(RS 18축부터 최악 ρ ≤ 0.003). FC·RU의 잔여 갭은 보완 택틱([`62`](62-방안2-보완-택틱-설계-측정.md))과 k 상한·의제 분할(K2)의 과제로 남긴다.
