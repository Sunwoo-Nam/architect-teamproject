...
> **갱신 4차 (2026-08-14)**: RU 사다리 20%p 등분 확정 → **5차(최종): [1%,100%] 로그 4등분 복귀** (24 §2.8 — 20%p가 저사용 구간 변별을 잃는 것이 FIN2 보고(70)에서 확인, PL 재확정). 현행 별점: 케이스 1 P95 = seq2 0.30% ★5 / pool 48.2% **★1**, 케이스 2 최대 = seq2 0.96% ★5 / pool 3,692% ★0.
...
| **2. 자원(메모리)** — r 케이스 1: **P95** | **0.30% ★5** | 48.2% ★1 | **방안 1** |
...
| RU | **사용률 r** = 단말 총 점유 ÷ 협상 몫 한도(128MB) — 별점은 **P95·최대값 2케이스 병행** (케이스 1 P95 = 꼬리 여유 · 케이스 2 최대 = 최악 안전, OOM 크리티컬. 단일 인용 대표 = 최대값) | ≤1% ★5 / ≤3.2% ★4 / ≤10% ★3 / ≤32% ★2 / ≤100% ★1 / 초과 결함 | **[1%,100%] 로그 4등분** — ★5 = 실사용 최대까지 성장(약 100배) 여유, 1칸 = √10 ≈ 3.2배 (20%p에서 복귀 — 24 §2.8 이력) | 2026-08-14 |
...
- RU에서 두 방안의 **중앙값은 동급 ★5**다 — 격차는 전부 규모 꼬리(P95·최악)에 있다. 두 케이스가 같은 방향을 가리키되(방안 1 우세), pool은 케이스 1(★1)과 케이스 2(★0 — "한 순간의 최악으로 죽는" OS 관점)의 간극으로 리스크의 성격까지 드러난다 (24 §2.6·§2.8).
...
- 로그 사다리(최종 — 20%p에서 복귀)가 저사용 자릿수 차이를 변별한다: seq2 0.96% ★5 vs P6 4.0% ★3 — 선형 20%p에서는 이 둘이 동급 ★5로 뭉개졌다 (사다리 이력: 24 §2.8).
...
- **Q. RU 판정 통계·사다리를 여러 번 바꾼 것이 방안 2에 불리한 조작 아닌가?** — 개정은 전부 24 §2.8에 근거와 함께 기록됐고, 방향도 일관되게 방안을 겨냥하지 않았다: 판정 통계(P95·최대 2케이스 병행)는 QA의 성격(OOM = 크리티컬은 최악 기준, TB = 체감은 P95)에서 나온 규칙이고, 한때의 20%p 등분(2026-08-14, 이후 로그 복귀)은 오히려 방안 2의 P95 등급을 ★1→★3으로 올리는 방향이었다. 두 케이스를 나란히 공개하므로 어느 한 통계의 선택이 결과를 왜곡할 여지도 없다 — 중앙값 판정이면 두 방안이 ★5 동급으로 뭉개져 방안 2의 실측 한도 초과 11건이 별점에 안 보이게 된다. 그쪽이 조작이다.
