...
> **갱신 3차 (2026-08-14, PL 지시)**: RU 별점을 **P95·최대값 2케이스 병행 산출·보고**로 확장 (24 §2.8 3차) — 케이스 1 P95 = 꼬리 여유(seq2 0.30% ★5 / pool 48.2% ★1), 케이스 2 최대 = 최악 안전(seq2 0.96% ★5 / pool 3,692% ★0). 두 별점의 간극이 곧 "평소 여유 vs 최악 리스크"의 정보다. 단일 인용 시 대표는 최대값(보수 기준). 측정값은 전 기간 불변.
...
| RU | **사용률 r** = 단말 총 점유 ÷ 협상 몫 한도(128MB) — 별점은 **P95·최대값 2케이스 병행** (케이스 1 P95 = 꼬리 여유 · 케이스 2 최대 = 최악 안전, OOM 크리티컬. 단일 인용 대표 = 최대값) | ≤1% ★5 / ≤3.2% ★4 / ≤10% ★3 / ≤32% ★2 / ≤100% ★1 / 초과 결함 | [1%,100%] 로그 4등분 — ★5 = 실사용 최대까지 성장(약 100배) 여유, 1칸 = √10 ≈ 3.2배 | 2026-08-14 |
...
- RU에서 두 방안의 **중앙값은 동급 ★5**다 — 격차는 전부 규모 꼬리(P95·최악)에 있다. 두 케이스가 같은 방향을 가리키되(방안 1 우세), pool은 케이스 1(★1)과 케이스 2(★0 — "한 순간의 최악으로 죽는" OS 관점)의 간극으로 리스크의 성격까지 드러난다 (24 §2.6·§2.8 3차).
...
- **Q. RU 판정을 최대값·로그 사다리로 바꾼 것이 방안 2에 불리한 조작 아닌가?** — 사다리 개정은 측정 전에 24 §2.8에 문서화됐고(근거: 선형 등분의 상·하 포화 실측), 판정 통계 개정(2026-08-14: 최대값 교체 → P95·최대 2케이스 병행)은 QA의 성격(OOM = 크리티컬 사건은 최악 기준, TB = 체감 품질은 P95)에서 나온 규칙이지 방안을 겨냥한 것이 아니다. 지금은 두 케이스를 나란히 공개하므로 어느 한 통계의 선택이 결과를 왜곡할 여지 자체가 없다 — 중앙값 판정이면 두 방안이 ★5 동급으로 뭉개져 방안 2의 실측 한도 초과 11건이 별점에 안 보이게 된다. 그쪽이 조작이다.
