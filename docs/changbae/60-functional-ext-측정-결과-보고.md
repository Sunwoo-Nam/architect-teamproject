...
> **판정 체계**: 2026-08-13 PL 확정 완료판 — FC = 달성률 · CF = 잔여 비밀률 · TB = ρ P95.
...
| **3. 기밀성** — 끝까지 비밀로 남는 내 선호 비율 | **68.3% ★4** | 51.7% ★3 | **방안 1-A** |
...
2. **기밀성·시간은 1-A의 구조적 우위.** 1-A의 잔여 비밀 68.3%는 1:1 협상 실측(70.8%)에 근접하고, 방안 2는 51.7%(케이스 중앙 기준 13.9%p 추가 노출). 시간 꼬리(P95)에서 방안 2는 소인원 판에서 naive에 근접(N=3에서 0.867)하는 반면 1-A는 최악도 0.681이다.
...
- **판정** = 1 − (나를 가장 많이 아는 관찰자의 깊이), 피해자 **평균** (중앙값은 1/12 격자에 스냅되어 N별 추이를 지움 — 24 §3.2).
- **모수 = 합의 완료 세션만** (합의율 80%, 두 방안 동일 — 병기 의무). 결렬 세션은 전 목록 소진이 본성이라 방안 변별 정보가 없다.
- **1:1 참조점**: 같은 판에서 2인 협상의 실측 잔여 = 70.8% (e₂ = 0.2917). "★4 이상 = 1:1 협상 수준"으로 읽는다.
...
| FR 위반 | 0건 | 0건 |
| CF 잔여 비밀률 (판정 — 합의 세션 384건 평균) | 68.3% ±19.4 **★4** | 51.7% ±23.7 **★3** |
...
| 기밀 쌍대: 잔여(1-A) − 잔여(2)의 중앙값 | +13.9%p — 합의 384케이스 중 294건에서 1-A가 더 지킴 |

## E. N(참여자 수)별 분석

| N | 1-A 달성률(★) | 2 달성률(★) | 승(1A/무/2) | 1-A 잔여비밀(★) | 2 잔여비밀(★) | 1-A ρP95(★) | 2 ρP95(★) |
|---|---|---|---|---|---|---|---|
| 3 | 0.938 (★4) | 0.954 (★5) | 11/24/25 | 80% (★5) | 71% (★4) | 0.606 (★2) | **0.867 (★1)** |
| 5 | 0.904 (★4) | 0.958 (★5) | 7/24/29 | 74% (★4) | 59% (★3) | 0.407 (★3) | 0.590 (★3) |
| 7 | 0.886 (★3) | 0.950 (★5) | 5/24/31 | 73% (★4) | 57% (★3) | 0.308 (★4) | 0.451 (★3) |
| 10 | 0.871 (★3) | 0.955 (★5) | 3/24/33 | 71% (★4) | 55% (★3) | 0.213 (★4) | 0.307 (★4) |
| 15 | 0.858 (★3) | 0.952 (★5) | 1/24/35 | 69% (★4) | 52% (★3) | 0.153 (★5) | 0.198 (★5) |
| 20 | 0.848 (★2) | 0.937 (★4) | 2/24/34 | 67% (★4) | 50% (★3) | 0.115 (★5) | 0.157 (★5) |
| 30 | 0.842 (★2) | 0.944 (★4) | 0/24/36 | 67% (★4) | 50% (★3) | 0.083 (★5) | 0.110 (★5) |
| 50 | 0.835 (★2) | 0.949 (★4) | 0/24/36 | 67% (★4) | 50% (★3) | 0.051 (★5) | 0.068 (★5) |

**FC**: 방안 2는 N 무관 0.94-0.96으로 평평(전원 제출 대기 구조), 1-A는 단조 하락 — 미끼 합의의 1인당 효용이 N과 함께 바닥선으로 수렴. **N=30부터 1-A 전패.**

**CF**: 격차는 전 구간 9-21%p로 안정. 1-A는 전 구간 "1:1 협상 수준(70.8%)" 근방을 지키고, 방안 2는 N≥20에서 50%에 붙는다. 편차는 방안 2가 큼(±23.7 vs ±19.4).

**TB**: 변별은 소인원에서 — N=3 1-A ★2 vs 방안 2 ★1(naive 근접 0.867). N=15부터 둘 다 ★5 수렴 (naive가 N 비례로 느려지는 반면 두 설계안은 병렬 제출 — I 참조).

## F. 유형별 분석 — 격차의 출처

| 유형 | 1-A 달성률(★) | 2 달성률(★) | 승(1A/무/2) | 1-A 잔여비밀(★) | 2 잔여비밀(★) |
|---|---|---|---|---|---|
| wide_common | 0.795 (★1) | 0.947 (★4) | 9/0/87 | 74% (★4) | 48% (★3) |
| utility_tradeoff | 0.671 (★0) | 0.803 (★2) | 20/0/76 | 64% (★4) | 49% (★3) |
| bottleneck | 0.897 (★3) | 1.000 (★5) | 0/0/96 | 78% (★4) | 55% (★3) |
| single_common | 1.000 (★5) | 1.000 (★5) | 0/96/0 | 57% (★3) | 56% (★3) |
...
- **유일해 판(single_common)은 CF도 동률**(57 vs 56%) — 교집합 소진 탐색의 구조적 하한.
...
|---|---|---|---|---|
| early (2-3위) | 0.851 | 0.933 | 82.2% | 73.6% |
| middle (4-7위) | 0.879 | 0.955 | 68.0% | 51.3% |
| late (8-10위) | 0.888 | 0.962 | 54.8% | **30.2%** |

FC는 깊이에 둔감(배치 편향 없음). **CF는 깊이에 강하게 의존** — 합의 후보가 깊을수록 전원이 순위를 더 꺼내야 하므로, late 판에서 방안 2의 잔여는 30%까지 떨어진다.
...
**Q4. 잔여 비밀률 0%가 없는 이유는?** — 분모가 전체 후보(12)인데 제출은 수락 가능 후보만 이뤄지므로, 거부 후보의 서열은 구조적으로 비밀로 남는다.
