...
> **2026-08-14 갱신**: [`53`](53-FR-재도출.md) 재검토로 FR 쪽 품질 요구 6건(구 FR-08·09·17·19·20·21)이 본 문서로 흡수·이동됨 — QAS-16·17 신설, QAS-01·03·10 시나리오·매핑 보강 ([`53`](53-FR-재도출.md) §53.1.1).
...
|---|---|---|---|---|
| QAS-01 | Functional Suitability — **Functional Correctness** | 벤치마크 협상을 실행했을 때, 시스템은 도달 가능한 가장 좋은 합의에 근접한 결과를 도출해야 한다 *(실현 불가능한 안의 배제 — [`09`](09-Functional-Correctness-정의.md) L2 — 를 포함)* | 달성률 U(r)/U(x\*) | PL-V5, PD-V2, SA-V1 |
...
| QAS-03 | Safety — **Operational Constraint** | 위임 범위 또는 사용자 수락 바닥선 밖의 안이 제안되었을 때, 시스템은 자동 수락을 차단하고 사용자 승인을 요청해야 한다 | 범위·바닥선 초과 자동 수락 0건 | SP-V1, EU-V2 |
...
| QAS-10 | Reliability — **Fault Tolerance** | 참여자 1명의 무응답·이탈 또는 외부 서비스 장애가 발생하더라도, 잔여 협상은 장애 전파 없이 정의된 결말로 완결되어야 한다 | 전파 없는 완결률 | OP-V2, OP-V5, SA-V3, SD-V1, SD-V4 |
...
| QAS-16 | Security — **Integrity** | 구성 요소가 선언된 권한 범위 밖의 데이터·기능에 접근을 시도하면, 시스템은 이를 차단해야 한다 | 권한 초과 접근 0건 | SA-V4, SP-V1 |
| QAS-17 | Security — **Integrity** | 참여자 위장 또는 메시지 변조 시도가 발생하면, 시스템은 이를 감지·차단해야 한다 | 위장·변조 미검출 0건 | SP-V2 |

## 54.2 QAS 계층화 및 중요도/난이도 평가 (사용자 확정, 2026-08-13 개정)

17건을 **핵심 4 · 준핵심 3 · 나머지**의 3계층으로 나눈다. **중요도는 계층 자체가 판정이므로 고정한다** — 핵심 H, 준핵심 M, 나머지 L. 변별은 난이도가 맡는다. 핵심의 서열과 등급은 사용자 확정, 준핵심 이하의 난이도는 분석자 평가다.
...
| QAS-16 | 권한 초과 접근 차단 | L/L | 선언 권한의 정적 검사·샌드박스는 표준 기법 (2026-08-14 추가, 구 FR-20) |
| QAS-17 | 위장·변조 감지 | L/L | 상호 인증·무결성 검증은 표준 암호 기법 (2026-08-14 추가, 구 FR-21) |
...
| QAS-16 → Security — Integrity (Safety 아님) | QAS-03(위임 경계)은 협상 **행위**의 운용 경계, QAS-16은 구성 요소의 데이터·기능 **접근 통제** — 보호 대상이 자산이므로 Security | 분석자 판단 (2026-08-14, 구 FR-20 이동분) |
| QAS-17 → Integrity (Authenticity 아님) | 상호 인증은 Authenticity 성격도 있으나 시나리오의 중심 보장이 위장·변조의 감지·차단이므로 Integrity를 주특성으로 | 분석자 판단 (2026-08-14, 구 FR-21 이동분) |
