...
## 54.2 QAS 계층화 및 중요도/난이도 평가 (사용자 확정, 2026-08-13 개정)

15건을 **핵심 4 · 준핵심 3 · 나머지**의 3계층으로 나눈다. **중요도는 계층 자체가 판정이므로 고정한다** — 핵심 H, 준핵심 M, 나머지 L. 변별은 난이도가 맡는다. 핵심의 서열과 등급은 사용자 확정, 준핵심 이하의 난이도는 분석자 평가다.

### 핵심 — 4건 (서열·등급 사용자 확정)

| 서열 | QAS | 항목 | 중요도/난이도 | 난이도 근거 |
|---|---|---|---|---|
| 1 | QAS-01 | 정확도 (FC) | **H/H** | 정답이 복수·판정 주체 N명이라 정의부터 어려움([`09`](09-Functional-Correctness-정의.md)). LLM 비결정성 위에서 달성 |
| 2 | QAS-06 | 자원 (RU) | **H/H** | ENV-A 대체 측정은 여유였으나 **정본 측정(실기기 RSS·온디바이스 LLM 동시 구동)은 미검증** — 온디바이스 상한 내 달성은 입증 전 |
| 3 | QAS-05 | 시간 (TB) | **H/M** | 합성 시간 측정 모델 기확립([`24`](24-QA-측정-핸드북.md)). 지연은 통신 왕복 수 설계로 조절 가능 |
| 4 | QAS-15 | 기밀성 (직접 공개) | **H/M** | FC와 트레이드오프 관계이나, 공개량 자체는 정보 공개 구조 설계로 통제 가능([`33`](33-정보-공개-구조.md)). 측정은 노출량 계수로 확립 |

### 준핵심 — 3건 (중요도 M 고정, 난이도 분석자 평가)

| QAS | 항목 | 중요도/난이도 | 난이도 근거 |
|---|---|---|---|
| QAS-08 | Scalability (의제 수) | M/**H** | 전수 물질화 실측: 10축×10 = ~69GB·협상 1건 ~31시간([`P5`](../../poc/dp-composite-agenda/results/P5-OOM규모-결과.md)) — 비자명한 설계 없이 달성 불가 |
| QAS-07 | Scalability (참여자 수) | M/M | 확장 지수 측정 체계 확립. 토폴로지 설계로 조절 가능함이 실측됨(dp2-nparty b 1.2~2.5) |
| QAS-10 | Fault Tolerance (참여자·외부) | M/M | 타임아웃·격리·대체 흐름은 표준 기법. 어려운 결합부(장애 시 종결 정합)는 QAS-02 소관 |

### 나머지 — 8건 (중요도 L 고정, 난이도 분석자 평가)

| QAS | 항목 | 중요도/난이도 | 난이도 근거 |
|---|---|---|---|
| QAS-02 | 종결 정합 | — | **핵심 1위(QAS-01)에 흡수 측정** — [`09`](09-Functional-Correctness-정의.md) 정본에서 FC의 L4 계층. 개별 등급 없음 |
| QAS-03 | 위임 초과 차단 | L/L | 결정론적 규칙 검사로 달성 — [`09`](09-Functional-Correctness-정의.md) L2 "기계 검증" |
| QAS-04 | 역추론 제한 | L/**H** | 패턴 역추론은 정보이론적 한계 — 방어·측정 모두 어려움 |
| QAS-09 | 복구 | L/M | 상태 저장·복원은 표준 기법. PoC REC 측정 기확립 |
| QAS-11 | 개입 반영 | L/M | 반영 자체는 단순하나 N명 전파 타이밍 이슈 |
| QAS-12 | 외부 전송 PII | L/M | 필터링은 표준 기법, 검출 완전성 입증이 관건 |
| QAS-13 | 재현성 | L/M | 하니스로 실증됨. LLM 비결정 재현만 난점 |
| QAS-14 | 모델 오출력 관용 | L/**H** | LLM 비결정성 — [`../08-DP_풀어야할문제.md`](../08-DP_풀어야할문제.md)가 지목한 근본 난제 |

**단서.**
1. QAS-14(모델 FT)는 준핵심의 "Fault Tolerance"에 포함하지 않았다 — 준핵심 FT는 참여자·외부 장애(QAS-10)를 가리키며, 모델 오출력은 성격이 달라 별도 유지한다.
2. QAS-08은 직전 판(2026-08-13 초판)에서 핵심이었으나 본 개정에서 준핵심으로 조정되었다(사용자 확정). 난이도 H 평가는 유지된다.
3. 계층은 측정 폐기가 아니다 — 준핵심·나머지도 감시·비교용 측정을 유지한다.
