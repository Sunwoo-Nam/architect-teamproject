...
| 27 | **Scalability-의제 수 정의·측정 (정본)** — 복합의제(multi-issue) 튜플 공간의 조합-메모리 탄력성 c, 별점 0-5점(전체 열거 1 - 이론 이상 1/d 등분) | [`27-Scalability-의제수-정의-측정.md`](27-Scalability-의제수-정의-측정.md) |
...
| 29 | **Confidentiality 정의·측정 (정본, 비핵심·보조 관측)** — frequency 공격자(자체 구현·규칙 봉인)의 정규화 노출률, 관점 2개(참여자/담당자) | [`29-Confidentiality-정의-측정.md`](29-Confidentiality-정의-측정.md) |
| 29-1 | **QA 측정 핸드북 (측정 수행 시 단일 참조 정본)** — 24-29 통합 + 미문서화 항목 추가(Time Behaviour의 phase 대체 지표, Message 전송 횟수·크기, Analysability), 관측량 3축(횟수=배터리·크기=대역·phase=시간) 정리 | [`29-1-QA-측정-핸드북.md`](29-1-QA-측정-핸드북.md) |
...
| 51 | **설계 후보 1 — 다자 합의 프로토콜 (구현·테스트 스펙)**: Blackboard + "라운드 k = 순위 k" 제출 공통 기반 위에서 방안 1 전원동의 투표형 / 방안 2 누적 공통제안형, tactic 파라미터 분리, 24·25 별점으로 비교 예정 | [`51-설계후보1-다자-합의-프로토콜.md`](51-설계후보1-다자-합의-프로토콜.md) |
