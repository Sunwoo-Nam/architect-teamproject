...
| 09 | (PL 지시) Functional Correctness 4계층 정의 확정 + **공정성의 별도 QA 분리 결론** | [`09-Functional-Correctness-정의.md`](09-Functional-Correctness-정의.md) |
...
- **Functional Correctness 결론** (09 §9.6): 1차 정의(각 참여자 관점의 의도 정확성∧제약 준수∧선호 정합∧실행 정합)로 확정하고, 2차(N명 간 분배)는 **별도 QA "협상 공정성"으로 분리**.
