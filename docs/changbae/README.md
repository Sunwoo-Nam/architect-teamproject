...
| 09 | (PL 지시) Functional Correctness 4계층 정의 확정 + **효용 분배 균형의 별도 지표 분리 결론** | [`09-Functional-Correctness-정의.md`](09-Functional-Correctness-정의.md) |
...
- **Functional Correctness 결론** (09 §9.6): 1차 정의(각 참여자 관점의 의도 정확성∧제약 준수∧선호 정합∧합의 종결 정합)로 확정하고, 2차(N명 간 분배)는 **별도 품질 지표 "효용 분배 균형"으로 분리**. 합의의 실행은 범위 밖 — L4는 "전원 동일 결과로 종결"까지.
