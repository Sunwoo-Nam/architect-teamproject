...
- **측정 체계** (10): QA ID 15개 항목 + QA-FC-2 + 기반 2개 항목 각각에 지표·목표 수준(절대 기준 vs 파일럿 목표)·테스트 환경(Mock 시뮬레이션 / 실기기 / 사용자 평가)·방법을 정의 — 하나의 QA 항목은 하나의 자극 부류·하나의 판정 유형만 가지며, 같은 지표를 두 항목에 이중 계상하지 않는다. QAS 작성의 원천.
