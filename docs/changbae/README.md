...
| 08 | 빈도 상위(4 이상) 핵심 QA 12종 집중 평가 → **최우선 QA 확정 (하위특성 11종 = QA ID 14개 항목)** + 설계 질문 9가지 | [`08-핵심-QA-중요도-난이도-평가.md`](08-핵심-QA-중요도-난이도-평가.md) |
...
| 10 | QA ID별(14개 항목 + QA-FC-2 + 기반 2개) **측정 지표·목표 수준·테스트 환경·방법** 정의 | [`10-핵심-QA-측정-계획.md`](10-핵심-QA-측정-계획.md) |
...
- **최우선 QA** (08 §8.3): 빈도 상위 12종 ∩ (H,H) 11종 = 교집합 9종 — 기능 정확성·결함 감내·확장성·시간 효율성·복구 가능성·시험 용이성·자원 효율성·기밀성·운용 제약 준수 — 에 (H,H) 단독 2종(공존성·운용성/사용자 통제)을 더한 하위특성 11종이 아키텍처 결정의 1차 드라이버. 측정·관리 단위로는 **QA ID 기준 14개 항목** — 자극·판정 유형이 다른 결함 감내(QA-FT-1·2), 확장성(QA-SC-1·2), 기밀성(QA-CF-1·2)은 각 2개 항목으로 분리.
