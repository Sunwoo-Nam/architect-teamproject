...
> **판정 체계**: FC = 달성률 · RU = 사용률 r의 **P95·최대값 2케이스 병행**([1%,100%] 로그 4등분 사다리 — 2026-08-14 최종, 20%p에서 복귀. 단일 인용 대표 = 최대값) · TB = ρ P95. 63과 동일 세션 상한(60스텝)·합성 상수.
...
| **2. 자원(메모리)** ① r P95 | **0.43% ★5** | 41.3% ★1 | **방안 1** |
| **2. 자원(메모리)** ② r 최대 (대표) | **1.4% ★4** | 550% ★0 | **방안 1** (한도 초과 0 vs 10건 — 전부 20축) |
| **4. 시간** — ρ P95 | 2.21 ★0 (결함 103건) | **0.579 ★3** | **방안 2** (T 중앙 8.4s vs 2.2s) |

**한 줄 결론**: 함정을 걷어내자 **FC 격차는 등급 동률(★4)로 좁혀졌고**(FIN 0.880★3 vs 0.941★4 → 0.922 vs 0.938), 구도는 "시간의 방안 2 vs 메모리의 방안 1"로 남는다. 방안 2의 메모리 결함(20축 초과 10건)은 **P6 택틱으로 전량 해소됨을 71이 실측**했으므로(pool+P6: r 최대 4.9% ★3·초과 0건·FC −0.004), 종합 우위는 **방안 2 + P6** — 적용 조건은 FIN의 "≤16축"에서 "**20축까지 전 구간**"으로 확장된다. seq2의 시간 결함(ρ>1 103건)은 함정과 무관하게 남았다 — 축별 세션 구조의 상수 비용이 원인이다(슬라이드 2).
...
2. **RU: 특이점이 사라지고 순수한 지수 성장 프로파일이 남았다.** pool의 r 최악은 12축 2.0% → 14축 8.2% → 16축 37.8% → 18축 **96.3%(한도 이내)** → 20축 550%로 **단조** — FIN에서 18축만 튀던 왜곡이 제거됐다. 한도 초과 10건은 전부 20축(k=2 풀 자체가 한도를 넘는 규모). seq2는 전 구간 1.4% 이하(★4-5 대역).
...
**의사결정 프레임**: FC가 동급이 된 무함정 구도에서는 서열 2위(RU)·4위(TB)가 가른다 — 메모리 결함(초과·★0)은 P6 택틱으로 해소 가능(71 실측 — ★3·초과 0)하고 시간 결함은 seq2에 구조적으로 남으므로, **방안 2 + P6**를 기본 선택으로 권고한다. 경로 함정형 도메인(축 간 봉쇄가 실재하는 문제)에서는 FIN 측정(63)이 보여준 함정 격차를 함께 고려해야 한다 — 두 측정은 대체가 아니라 상보다(J 참조).
...
63 A절과 동일하다 (정본 24 §1·§2·§4). 판정 체계는 24 §0 최종판 — FC 달성률 · RU [1%,100%] 로그 사다리 + P95·최대 2케이스 병행 · TB ρ P95. (측정 시점에는 RU가 한때의 20%p 등분이었으나 같은 날 로그로 복귀 확정 — 별점은 원값 불변 재환산이라 측정 유효성에 영향 없다. 이력: 24 §2.8.)
...
| FR 위반 케이스 | 0 | 0 |
| **RU r — 케이스 1: P95** | 0.43% **★5** | 41.3% **★1** |
| **RU r — 케이스 2: 최대값** (단일 인용 대표) | 1.4% **★4** | 550% **★0** |
...
- FIN 대비 등급 변화: seq2 FC ★3→**★4**·TB P95 4.24→2.21(★0 유지), pool TB ★2→**★3**·RU 케이스1 ★1(48.2→41.3% — 등급 동일). seq2 RU 케이스2는 0.96%★5→1.4%★4 (한 칸 경계 1% 직상). 방향 전부 "함정 왜곡 제거"와 정합.
...
| 12 | 0.931 | 0.936 | 2.77 | 0.43 | 0.44% | 1.43% | 0.203MB |
...
### E2. 축별 종합 — FC · RU(P95·최대) · TB(ρ P95 + 실제 시간, naive 포함)

F2(카테고리별)와 동일 규약의 축 수준별 분해다. 시간은 합성 시간 모델의 절대값, naive T는 방안 무관(케이스의 성질). 12축은 CR(오라클 판)과 RS(스트레스 판)를 분리 표기한다 — 표본 성질이 다르다.

**방안 1 (seq2):**

| 축 | FC | r P95 | r 최대 | ρ P95 | ρ 최악 | T 중앙 | T P95 | naive T 중앙 | naive T P95 |
|---|---|---|---|---|---|---|---|---|---|
| CR 4 | 0.954 | 0.23% | 0.8% | 3.05 | 6.66 | 3.8s | 9.0s | 4.1s | 176.1s |
| CR 5 | 0.908 | 0.21% | 0.5% | 3.34 | 4.46 | 5.7s | 23.9s | 5.0s | 69.6s |
| CR 6 | 0.913 | 0.19% | 0.3% | 2.04 | 9.45 | 6.0s | 16.5s | 12.7s | 236.8s |
| CR 7 | 0.923 | 0.23% | 0.7% | 2.06 | 3.27 | 7.3s | 16.0s | 29.4s | 432.8s |
| CR 8 | 0.911 | 0.40% | 0.7% | 1.66 | 2.65 | 9.0s | 32.0s | 36.0s | 823.8s |
| CR 9 | 0.916 | 0.51% | 1.0% | 1.93 | 2.62 | 9.0s | 49.4s | 27.4s | 1,106.3s |
| CR 10 | 0.913 | 0.27% | 0.5% | 1.83 | 2.70 | 10.8s | 21.5s | 29.8s | 839.2s |
| CR 11 | 0.928 | 0.41% | 0.7% | 1.82 | 3.29 | 11.7s | 29.6s | 22.7s | 766.2s |
| CR 12 | 0.931 | 0.44% | 1.3% | 2.77 | 6.11 | 14.7s | 34.6s | 21.6s | 369.6s |
| RS 12 | — | 0.42% | 0.4% | 0.51 | 0.51 | 11.8s | 26.5s | 112.8s | 6,355.1s |
| RS 14 | — | 1.39% | 1.4% | 0.12 | 0.12 | 15.2s | 117.7s | 1,512.8s | 39,539.0s |
| RS 16 | — | 1.13% | 1.1% | 0.05 | 0.05 | 20.3s | 93.4s | 2,127.9s | 29,244.4s |
| RS 18 | — | 1.10% | 1.1% | 0.01 | 0.01 | 19.7s | 86.5s | 27,513.1s | 124,623.2s |
| RS 20 | — | 0.95% | 1.0% | 0.00 | 0.00 | 22.6s | 77.9s | 184,917.0s | 978,417.6s |

**방안 2 (pool):**

| 축 | FC | r P95 | r 최대 | ρ P95 | ρ 최악 | T 중앙 | T P95 | naive T 중앙 | naive T P95 |
|---|---|---|---|---|---|---|---|---|---|
| CR 4 | 0.950 | 0.15% | 0.2% | 1.21 | 1.54 | 1.6s | 9.0s | 4.1s | 176.1s |
| CR 5 | 0.933 | 0.13% | 0.2% | 1.00 | 1.71 | 2.2s | 9.0s | 5.0s | 69.6s |
| CR 6 | 0.930 | 0.12% | 0.3% | 0.46 | 1.16 | 1.9s | 9.0s | 12.7s | 236.8s |
| CR 7 | 0.938 | 0.19% | 0.3% | 0.51 | 0.63 | 2.2s | 9.0s | 29.4s | 432.8s |
| CR 8 | 0.946 | 0.32% | 0.5% | 0.37 | 0.58 | 2.1s | 9.0s | 36.0s | 823.8s |
| CR 9 | 0.929 | 0.37% | 1.7% | 0.39 | 0.52 | 2.1s | 9.0s | 27.4s | 1,106.3s |
| CR 10 | 0.936 | 0.42% | 0.5% | 0.40 | 0.56 | 2.3s | 9.0s | 29.8s | 839.2s |
| CR 11 | 0.942 | 0.52% | 1.0% | 0.28 | 1.03 | 2.2s | 9.0s | 22.7s | 766.2s |
| CR 12 | 0.936 | 1.43% | 1.5% | 0.43 | 0.70 | 2.4s | 9.0s | 21.6s | 369.6s |
| RS 12 | — | 2.04% | 2.0% | 0.08 | 0.08 | 2.3s | 9.0s | 112.8s | 6,355.1s |
| RS 14 | — | 8.24% | 8.2% | 0.03 | 0.03 | 2.5s | 9.0s | 1,512.8s | 39,539.0s |
| RS 16 | — | 37.80% | 37.8% | 0.00 | 0.00 | 3.0s | 9.0s | 2,127.9s | 29,244.4s |
| RS 18 | — | 96.31% | 96.3% | 0.00 | 0.00 | 2.2s | 9.1s | 27,513.1s | 124,623.2s |
| RS 20 | — | **550.06%** | **550.1%** | 0.00 | 0.00 | 2.5s | 8.2s | 184,917.0s | 978,417.6s |

읽는 법 3가지:

1. **RS 수준은 표본이 20건이라 r P95 ≈ r 최대**다 (분위 색인상 일치) — RS의 두 컬럼이 같은 것은 데이터 중복이 아니라 표본 크기의 귀결. CR(44-45건)에서는 둘이 갈린다 (예: pool 9축 P95 0.37% vs 최대 1.7%).
2. **naive T는 축 수에 지수로 자란다**: CR에서 4.1s → 20-36s, RS에서 112.8s → 184,917s(약 51시간, 20축 중앙). 두 설계안은 전 구간 1.5-22.6초 — ρ가 규모에서 0으로 붕괴하는 것은 설계가 빨라져서가 아니라 naive가 못 버티는 것이다.
3. **seq2의 T는 축 수에 비례**(축별 세션 — CR 3.8→14.7s), **pool의 T는 평평**(전 축 한 세션 — 1.6-3.0s). seq2 ρ>1이 소규모 CR에 몰리는 이유가 이 교차에 있다: naive T가 아직 작은 구간(4-6축, 4.1-12.7s)에서 seq2의 세션 비용이 그걸 넘는다.

**그림 E-1 — 축 수별 실제 시간(합성 T, 로그 스케일).** 실선+마커 = 중앙값, 점선 = P95. CR 패널에서 seq2(파랑)와 naive(회색)의 교차(5-6축)가 seq2 ρ>1 결함의 발생 구간이고, RS 패널에서 naive는 10^5초(수십 시간) 대역으로 폭발하는 반면 설계안들은 10초 아래에 평평하다:

![축 수별 실제 시간](figures/70/fig-time-by-axis.png)

> 생성: [`plot_time_by_axis.py`](../../poc/total/scripts/plot_time_by_axis.py) (cases.jsonl 재파생 — P6 곡선 포함, 71 공용)
...
### F2. 카테고리별 종합 — FC · RU(P95·최대) · TB(ρ P95 + 실제 시간, naive 포함)

데이터셋 카테고리(유형)별로 세 QA의 판정 통계와 **실제 시간**을 한 표에 정리한다. 시간은 벽시계가 아니라 **합성 시간 모델**(24 §4.4 — phase×75ms + 평가 + 전송, 재현 가능)의 절대값이며, ρ = T ÷ T_naive의 분모인 **naive의 실제 시간도 병기**한다 (naive T는 방안 무관 — 케이스의 성질이다).

**방안 1 (seq2):**

| 카테고리 | 건수 | FC | r P95 | r 최대 | ρ P95 | ρ 최악 | T 중앙 | T P95 | naive T 중앙 | naive T P95 |
|---|---|---|---|---|---|---|---|---|---|---|
| plain | 355 | 0.912 | 0.29% | 1.3% | **2.65** | 9.45 | 7.7s | 20.3s | 15.1s | 255.6s |
| no_deal | 45 | 1.000 | 0.68% | 0.8% | 0.76 | 2.45 | 9.0s | 56.4s | 270.3s | 2,176s |
| stress | 100 | — | 0.89% | 1.4% | 0.32 | 0.51 | 16.7s | 75.4s | 5,507s | 385,284s |

**방안 2 (pool):**

| 카테고리 | 건수 | FC | r P95 | r 최대 | ρ P95 | ρ 최악 | T 중앙 | T P95 | naive T 중앙 | naive T P95 |
|---|---|---|---|---|---|---|---|---|---|---|
| plain | 355 | 0.930 | 0.59% | 1.5% | 0.64 | 1.71 | 2.0s | 3.9s | 15.1s | 255.6s |
| no_deal | 45 | 1.000 | 0.51% | 1.7% | 0.21 | 0.82 | 9.0s | 9.0s | 270.3s | 2,176s |
| stress | 100 | — | **228.9%** | **550%** | 0.03 | 0.08 | 2.5s | 9.0s | 5,507s | 385,284s |

읽는 법 4가지:

1. **seq2의 TB 결함은 전부 plain에서** 난다 (ρ P95 2.65 — no_deal 0.76·stress 0.32는 결함 구간 밖). 소규모 일반 판에서 naive가 이미 빠른데(중앙 15.1s) seq2는 축별 세션 상수 비용(중앙 7.7s, P95 20.3s)이 그걸 넘는 꼬리를 만든다.
2. **naive의 실제 시간이 카테고리의 성질을 정의한다**: plain 15초 → no_deal 270초(결렬 증명 = 전수 소진) → stress 5,507초(중앙, 약 92분 — P95는 4.5일). 설계안 두 개는 stress에서도 2.5-16.7초로 끝난다 — "전체 공간을 만들지 않는 설계"의 존재 이유가 절대 시간으로 드러난다 (ρ가 규모에서 0으로 떨어지는 이유이기도 하다).
3. **RU 리스크는 pool×stress 한 칸에 몰려 있다** (r P95 228.9%·최대 550%) — 다른 다섯 칸은 전부 2% 미만. 이 한 칸이 P6 택틱의 정확한 표적이다 (71 — 적용 후 1.94%·4.9%).
4. no_deal의 T가 양 방안 9.0초(상한 60스텝 소진)로 같다 — 결렬 인지의 비용은 방안이 아니라 세션 상한이 정한다.

### F3. 실제 시간(T) 통계 상세 — 최소 · 평균 · 중앙 · P95 · 최대

카테고리별 합성 T의 전 통계다 (naive는 방안 무관 — baseline 성립 케이스 기준). **평균이 중앙값보다 훨씬 큰 것이 naive의 특징**이다 — 소수의 대형 판이 평균을 끌어올리는 극단 꼬리 분포라, naive와의 비교는 중앙·P95로 읽는 것이 안전하다.

**plain (355건):**

| | 최소 | 평균 | 중앙 | P95 | 최대 |
|---|---|---|---|---|---|
| seq2 | 0.7s | 9.5s | 7.7s | 20.3s | 94.1s |
| pool | 0.2s | 2.1s | 2.0s | 3.9s | 7.5s |
| naive | 0.2s | 56.5s | 15.1s | 255.6s | 1,579.5s |

**no_deal (45건):**

| | 최소 | 평균 | 중앙 | P95 | 최대 |
|---|---|---|---|---|---|
| seq2 | 0.0s | 20.0s | 9.0s | 56.4s | 67.6s |
| pool | 0.0s | 8.4s | 9.0s | 9.0s | 9.0s |
| naive | 11.0s | 978.1s | 270.2s | 2,176.0s | 14,542.1s |

**stress (100건):**

| | 최소 | 평균 | 중앙 | P95 | 최대 |
|---|---|---|---|---|---|
| seq2 | 4.7s | 21.8s | 16.7s | 75.4s | 117.7s |
| pool | 0.5s | 3.1s | 2.5s | 9.0s | 9.1s |
| naive | 17.2s | **63,023.8s** | 5,507.0s | 385,284.0s | **978,417.6s** |

읽는 법: ① pool의 no_deal·stress에서 P95 = 최대 ≈ 9.0s는 세션 상한(60스텝 × 75ms × 2phase)의 천장이다 — 분포가 아니라 설계 파라미터가 만든 값. ② naive stress의 평균(63,024s ≈ 17.5시간)이 중앙(5,507s)의 11배 — 20축 꼬리가 평균을 지배한다. ③ seq2의 최소 0.0s(no_deal)는 첫 축에서 즉시 결렬을 인지한 판이다.
...
- **Q. FC가 등급 동률이 됐는데 왜 여전히 방안 2인가?** — 원값 +0.016·승패 172:157의 근소 우위에 더해, 서열 4위 TB에서 방안 2는 ★3, 방안 1은 결함(★0)이다. 방안 1이 이기는 축은 RU뿐인데 그 결함은 P6 택틱이 소거한다(71: 초과 0건·r 최대 4.9% ★3·FC 비용 −0.004 — 등급 한 칸 열세는 남되 결함 아님). 반면 방안 1의 TB 결함은 축별 세션의 구조 비용이라 대응 택틱이 실측되지 않았다(66에서 S 계열 전부 PL 기각).
